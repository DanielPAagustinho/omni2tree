...
| `--exclude_pattern` | Regex to exclude sequence labels during entropy table generation. Examples: `s0`, `RespiratorysyncytialvirusA`, `MinION_18_SRR33779449`, or a regex such as `^(MinION|NextSeq)_` to match multiple readsets. |
...
- `label`: Reference/readset name used for Step 3 matching. Reference labels must match the taxon/label/strain names used in Step 1 (`-i/--input` or `-L/--local_assemblies`). Readset labels must match exactly the consensus readset names detected from `O2T_RESULTS/*_all_cov.txt`.
- `identifier`: Stable row ID for the viewer. It can be an NCBI accession, a local FASTA filename/stem, an SRA run, or another unique identifier. Step 3 does not use it to match Step 1 references; matching is done with `label`.

Validation rules:
- `label` and `identifier` cannot be empty, contain commas, or be duplicated.
- Metadata column names are case-insensitive and cannot be duplicated.
- Do not add a `source` column or any column to mark rows as reference/readset; Step 3 detects this automatically.
- Labels must not collide after alphanumeric/tree-label cleanup.
...
```csv
label,identifier,Genotype,Host
