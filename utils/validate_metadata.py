...
def validate_header_collisions(header: List[str]) -> None:
    normalized = [col.strip().lower() for col in header]
    duplicates = sorted({col for col in normalized if normalized.count(col) > 1})
    if duplicates:
        raise ValueError(
            "Metadata has duplicated column name(s), case-insensitive: "
            + ", ".join(duplicates)
        )

    reserved = sorted(VIEW_RESERVED_COLUMNS.intersection(normalized))
    if reserved:
        raise ValueError(
            "Metadata column name(s) conflict with columns generated by step3 for "
            "omni2treeview: "
            + ", ".join(reserved)
            + ". Rename them in the input metadata before running step3."
        )
...
def parse_metadata_csv(path: Path) -> MetadataInput:
    ...
    validate_header_collisions(header)
