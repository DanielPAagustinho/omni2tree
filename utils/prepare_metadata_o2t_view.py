...
- Required metadata columns: label, identifier (case-insensitive).
- Output metadata uses identifier as the first Omni2treeView metadata column.
"""
...
def parse_metadata_csv(path: Path) -> MetadataInput:
    ...
    forbidden_cols = {"source"}.intersection(lower_map)
    if forbidden_cols:
        raise ValueError(
            "Metadata column name(s) conflict with columns generated by step3 for "
            "omni2treeview: "
            + ", ".join(sorted(forbidden_cols))
        )
...
def build_output_metadata(
    ...
    extra_cols = [c for c in meta.header if c.lower() not in {"label", "identifier"}]
    extra_types = [meta.types[meta.header.index(c)] for c in extra_cols]

    out_header = ["identifier", "label", "source"] + extra_cols
    ...
    for row in meta.rows:
        ...
        out_rows.append([identifier, label_final, source] + extras)
